...
import static com.centit.support.database.utils.QueryUtils.buildLimitQuerySQL;
...
public class TenantMemberDao extends BaseDaoImpl<TenantMember, String> {
    ...
    /**
     * 分页查询租户内成员信息
     *f_userinfo 和work_group 关联是为了通过ROLE_CODE进行区分过滤
    ...
    public List<TenantMember> pageListTenantMember(TenantMemberQo tenantMemberQo, PageDesc pageDesc) {
        ...
        String querySql = buildLimitQuerySQL(selectFieldsSql + fromAndFilterSql,
            pageDesc.getRowStart(), pageDesc.getPageSize(), false, this.getDBtype());
